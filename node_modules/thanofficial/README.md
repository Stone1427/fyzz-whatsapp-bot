...
```bash
npm install thanofficial 
```

```bash
yarn add thanofficial 
```

```bash
pnpm add thanofficial 
```
...
```javascript
import makeWASocket from 'thanofficial '
```

**Untuk proyek dengan CJS (CommonJS):**
```javascript
const makeWASocket = require('thanofficial ')
```

thanofficial  secara otomatis mendukung ESM dan CJS tanpa perlu konfigurasi tambahan.

## Informasi

Saat ini, `thanofficial ` membutuhkan Node.js versi **20 atau lebih tinggi** untuk berjalan.
...
```javascript
import makeWASocket, { Browsers } from 'thanofficial '
...
```javascript
import makeWASocket, { useMultiFileAuthState } from 'thanofficial '
...
```javascript
import makeWASocket, { DisconnectReason, useMultiFileAuthState } from 'thanofficial '
...
const collection = client.db('thanofficial ').collection('sessions')
...
const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: 'thanofficial ' })
logger.level = 'fatal'
...
```javascript
import makeWASocket, { makeInMemoryStore } from 'thanofficial '
...
import { downloadMediaMessage, getContentType } from 'thanofficial '
